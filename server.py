import asyncio,json,uuid,logging,websockets,random,time,os
...
async def call_llm(messages,providers=None):
    ...
    for p in active:
        try:
            ...
            if "openai" in url.lower() or name=="OpenAI":
                pass
            ...
            async with httpx.AsyncClient(timeout=30) as c:
                ...
                async for l in r.aiter_lines():
                    if l.startswith("data: ")and l[6:]!="[DONE]":
                        try:full+=json.loads(l[6:])["choices"][0]["delta"].get("content","")
                        except:pass
                if full:return full
...
- 只回答问题，不延伸"""
    ...
    active_models=[p["name"]+":"+p["selected"] for p in CFG.get("llm_providers",[])if p.get("enabled")and p.get("api_key")]
...
@app.get("/")
async def root():return{"status":"ok","name":"QinYi","ver":"5.0","online":len(sessions)}
...
async def gc():
    ...
    for k in ["llm_providers","mcp_servers"]:
        if k=="llm_providers":
            ...
            for p in CFG.get(k,[]):
                cp=dict(p)
                if cp.get("api_key"):cp["api_key"]="••••••"
                pub[k].append(cp)
        elif k=="mcp_servers":
            pub[k]=[]
            for m in CFG.get(k,[]):
                cm=dict(m)
                if cm.get("token"):cm["token"]="••••••"
                pub[k].append(cm)
    return pub
...
async def stats():return{
    ...
    "uptime":time.time()-start_time,"version":"5.0",
    "active_sessions":[{"id":s.id,"msg_count":s.msg_count}for s in sessions.values()],
}

@app.get("/config")
async def cfg():
    return responses.HTMLResponse(r"""<!DOCTYPE html>
<html lang="zh-CN">
...
.card-desc{font-size:11px;color:#777;margin-top:2px}
...
.back{display:inline-flex;align-items:center;gap:6px;color:#aaa;font-size:15px;cursor:pointer;padding:12px 16px;margin-bottom:8px;border-radius:8px;background:rgba(255,255,255,0.04);border:1px solid rgba(255,255,255,0.06);width:fit-content;min-width:80px;font-weight:500;transition:all 0.15s}
.back:hover{color:#00d2ff;background:rgba(0,210,255,0.08);border-color:rgba(0,210,255,0.2)}
.back:active{transform:scale(0.97)}
.back .arr{font-size:20px;margin-right:2px}
...
<div class="step"><span class="step-num">3</span><strong style="color:#ccc">连接服务器</strong><br><span style="color:#888;font-size:12px">自动连接 ws://你的服务器IP:8001，无需激活码或绑定，连上即用。</span></div>
<div class="step"><span class="step-num">4</span><strong style="color:#ccc">开始对话</strong><br><span style="color:#888;font-size:12px">说「你好琴一」唤醒 → 说话 → 琴一回答。</span></div>
...
<button class="modal-close" onclick="hideHelp()">知道了</button>
    ...
    <div class="card-left"><div class="card-title">🧠 基础信息 <span class="tag tag-new">NEW</span></div><div class="card-desc">机器人名字 · 对你的称呼</div></div>
    ...
    <div class="card-left"><div class="card-title">🔑 API密钥与模型 <span class="tag tag-hot">多供应商</span></div>
    ...
    <div class="card-left"><div class="card-title">🔌 MCP扩展 <span class="tag tag-hot">多服务器</span></div>
...
function renderApi(data){
    let html=`<div class="back" onclick="showPage('main')"><span class="arr">←</span> 返回</div>
    ...
    (data.llm_providers||[]).forEach((p,i)=>{
        ...
        <label>显示名称</label>
        <input value="${p.name||''}" onchange="updateProvider(${i},'name',this.value)">
        <label>API 调用地址</label>
        <input value="${p.api_url||''}" placeholder="https://api.deepseek.com" onchange="updateProvider(${i},'api_url',this.value)">
        <label>API Key</label>
        ...
        <input value="${p.models||''}" placeholder="deepseek-chat,deepseek-reasoner" onchange="updateProvider(${i},'models',this.value)">
        <div class="gr">
        <div><label>当前选用</label><input value="${p.selected||''}" placeholder="deepseek-chat" onchange="updateProvider(${i},'selected',this.value)"></div>
        ...
        <input value="${p.headers||''}" placeholder='{"anthropic-version":"2023-06-01"}' onchange="updateProvider(${i},'headers',this.value)">
        </div>`;
    });
...
function renderMcp(data){
    ...
    (data.mcp_servers||[]).forEach((m,i)=>{
        ...
        <label>名称</label>
        <input value="${m.name||''}" placeholder="HomeAssistant" onchange="updateMcp(${i},'name',this.value)">
        <label>连接地址</label>
        <input value="${m.url||''}" placeholder="ws://192.168.1.100:8123/mcp" onchange="updateMcp(${i},'url',this.value)">
...
main:(d)=>renderMain(d),
basic:(d)=>`
<div class="back" onclick="showPage('main')"><span class="arr">←</span> 返回</div>
...
async def wss():
    ...
    async with websockets.serve(h,"0.0.0.0",8001):
        log.info("✅ 琴一 v5.0 | WS:8001 | HTTP:8000")
